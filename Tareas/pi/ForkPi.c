...
/*		struct foo * pi2;
	...
	int main(int argc, char * argv[])
	{
		...
		for(i = 0; i < MAXHILOS; i++)
		{
			...
			if(r == 0)	{ printf("Hilo creado \n"); }
			else		{ perror("Error en el hilo"); }
		}
		...
		for(i = 0; i < MAXHILOS; i++)
		{
			...
			if(r == 0)	{ printf("Fin de hilo"); }
			else		{ perror("Error en el fin de hilo"); }
		}

		for(i = 0; i < MAXHILOS; i++)
		{
			pi[0]->resultado += pi[i + 1]->resultado;
		}
